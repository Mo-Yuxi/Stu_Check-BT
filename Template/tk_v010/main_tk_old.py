...
def One():
    if (grade_input.get() == '' or profession_input.get() == ''
        ...
        Entry_out.insert('insert', "请先输入正确的信息创建学生名单。\n")
        ...
        Entry_out.insert('insert', strReturn)
...
def Three():
    if (stu1.grade != -1 or stu1.classNo != -1):
        ...
        Entry_out.insert('insert', "学生名单不存在，请先加载或创建学生名单。\n")
...
def Scan():
    Entry_out.insert('insert', "扫描中，请等待...\n")
...
bott_getin = tkinter.Button(window, text="扫描", command=Scan)
