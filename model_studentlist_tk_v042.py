...
class StudentList:
    ...
    def checkAddr(self, located_list: list):
        """将实时列表与存储列表进行比较并列出可疑设备"""
        strTemp = ''
        suspDevices = []
        localTemp = located_list
        for (idF1, nameF1, addrF1) in localTemp:
            for (idF2, nameF2, addrF2) in localTemp:
                if addrF1 == addrF2 and (idF1 != idF2 or nameF1 != nameF2):
                    if (idF1, nameF1,
                            addrF1) in suspDevices or (idF2, nameF2,
                                                       addrF2) in suspDevices:
                        suspDevices.append((idF1, nameF1, addrF1))
                        suspDevices.append((idF2, nameF2, addrF2))
    ...
    def checkList(self, located_list: list):
        """通过学生列表进行签到统计并列出缺席学生"""
        strTemp = ''
        self.absentStu = []
        stutemp = []
        ...
        for stuLocated in located_list:
            loctemp.append(stuLocated)
        for stup in stutemp:
            if stup not in loctemp:
                self.absentStu.append(stup)
                strTemp += "{}未签到！\n".format(stup[1])
        strTemp += "\n"
