...
import tkinter
import threading
from tkinter import Entry, StringVar, scrolledtext
from model_studentlist_tk_v042 import StudentList
...
window.title('蓝牙学生点名器v0.4.2    Designed By 2019级物联网工程开源大帝组')
...
e.set("本软件由2019级物联网工程 黄山 袁茂涵 蒋明君 开发！\n")
print(e.get())'''
L1 = tkinter.Label(window,
                   text='本软件由2019级物联网工程 黄山 袁茂涵 蒋明君 开发！\n',
                   font=("宋体", 11))
L1.place(x=80, y=0)
    ...
    if lenTemp == 0:
        for (addr1, name1) in devices_list:
            if ("#1#." in name1):
                ...
                see_list.append(addTemp)
    elif lenTemp != 0:
        for (addr1, name1) in devices_list:
            for located in located_list:
                if (name1 != located[1] and "#1#." in name1):
                    addTemp = [name1[4:14], name1[14:], addr1]
                    located_list.append(addTemp)
                    see_list.append(addTemp)
                    break
...
bott_getin = tkinter.Button(window, text="扫描名单", command=show)
bott_getin.place(x=30, y=120, width=60, height=30)
